...
SELECT geo.GeoPietro.id_pietro, geo.GeoPietro.nazwa_pietro, GeoEpoka.id_epoka,
GeoEpoka.nazwa_epoka, GeoOkres.id_okres, GeoOkres.nazwa_okres, GeoEra.id_era,
GeoEra.nazwa_era, GeoEon.id_eon, GeoEon.nazwa_eon
INTO GeoTabela
FROM geo.GeoPietro
JOIN geo.GeoEpoka ON GeoEpoka.id_epoka = geo.GeoPietro.id_epoka
JOIN geo.GeoOkres ON GeoOkres.id_okres = GeoEpoka.id_okres
JOIN geo.GeoEra ON GeoEra.id_era = GeoOkres.id_era
JOIN geo.GeoEon ON GeoEon.id_eon = GeoEra.id_eon;
...
SELECT COUNT(*) 
FROM Milion 
INNER JOIN GeoTabela ON Milion.liczba % 77 = GeoTabela.id_pietro;
...
INNER JOIN geo.GeoPietro ON (Milion.liczba % 77=GeoPietro.id_pietro)
...
-- indeksowanie
CREATE INDEX iEon ON geo.GeoEon(id_eon);
CREATE INDEX iEra ON geo.GeoEra(id_era, id_eon);
CREATE INDEX iOkres ON geo.GeoOkres(id_okres, id_era);
CREATE INDEX iEpoka ON geo.GeoEpoka(id_epoka, id_okres);
CREATE INDEX iPietro ON geo.GeoPietro(id_pietro, id_epoka);
