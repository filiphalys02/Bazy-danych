...
SELECT COUNT(*)
FROM Milion
    INNER JOIN GeoTabela ON (mod(Milion.liczba,77)=(GeoTabela.id_pietro));
...
SELECT COUNT(*)
FROM Milion
WHERE mod(Milion.liczba,77)= (SELECT id_pietro FROM GeoTabela   WHERE mod(Milion.liczba,68)=(id_pietro));


-- zapytanie nr 4
SELECT COUNT(*)
FROM Milion
WHERE mod(Milion.liczba,77) IN (SELECT geo.GeoPietro.id_pietro FROM geo.GeoPietro
    NATURAL JOIN geo.GeoEpoka
    NATURAL JOIN geo.GeoOkres
    NATURAL JOIN geo.GeoEra
    NATURAL JOIN geo.GeoEon);
...
DROP INDEX geo.indxEon;
DROP INDEX geo.indxEra;
DROP INDEX geo.indxOkres;
DROP INDEX geo.indxEpoka;
DROP INDEX geo.indxPietro;
DROP INDEX indxLiczba;
